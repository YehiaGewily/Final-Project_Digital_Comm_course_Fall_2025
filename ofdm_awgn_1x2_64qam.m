%% OFDM Project - Phase 5A: SIMO 1x2 AWGN (64-QAM)
% 1 Tx -> 2 Rx antennas
% Channel: AWGN only (no fading)
% Receiver: Equal Gain Combining (Summing) -> Demap -> BER
% Coding: Hamming (7,4)
...
for e = 1:length(EbNo_dB)
    ...
    for run = 1:numRuns
        ...
        % Constraint 2: Must be multiple of k0 (6) for 64-QAM Modulator
        % LCM(7, 6) = 42. So we floor to nearest multiple of 42.
        nCodedBitsNeeded = floor(nCodedBitsAvailable/42) * 42;
        
        % Calculate required raw bits
        ...
        % Add Independent AWGN to both branches
        n1 = sqrt(noisePower/2) * (randn(size(tx_serial)) + 1i*randn(size(tx_serial)));
        n2 = sqrt(noisePower/2) * (randn(size(tx_serial)) + 1i*randn(size(tx_serial)));
        
        rx1_serial = tx_serial + n1;
        rx2_serial = tx_serial + n2;
        ...
        % 2. Equal Gain Combining (Time Domain)
        % Sum the branches and divide by 2 to average the noise
        rx_combined = (rx1_no_cp + rx2_no_cp) / 2;
        
        % 3. FFT
        Yf = fft(rx_combined, N, 1);
...
%% Plot Results
figure;
semilogy(EbNo_dB, BER, 'k-d', 'LineWidth', 2, 'MarkerFaceColor', 'k');
grid on;
title('SIMO 1x2 AWGN (64-QAM) with (7,4) Coding');
xlabel('Eb/N0 (dB)');
ylabel('Bit Error Rate (BER)');
legend('SIMO 1x2 AWGN 64-QAM');
ylim([1e-5 1]);
