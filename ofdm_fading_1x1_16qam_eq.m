...
%% Main simulation (Updated for (7,4) Channel Coding)
for e = 1:length(EbNo_dB)
    ...
    for run = 1:numRuns
        ...
        txDataBits = randi([0 1], numRawBits, 1); % Original info bits

        % 2. ENCODE the bits using (7,4) Linear Block Code [cite: 82, 99]
        ...
        % 4. OFDM Modulation (IFFT) and Cyclic Prefix [cite: 93, 119-121]
        x_time = ifft(X, N, 1);
        ...
        %% ---------- BIT ERROR RATE (BER) ----------
        % Compare decoded bits to original raw bits [cite: 23-24]
