%% OFDM Project - Phase 5A: SISO AWGN (QPSK)
% 1 Tx -> 1 Rx antenna
% Channel: AWGN only (no fading)
% Coding: Hamming (7,4)
% Expected Result: Standard waterfall BER curve for Coded QPSK in AWGN.
...
for e = 1:length(EbNo_dB)
    ...
    for run = 1:numRuns
        ...
        % Constraint 2: Must be multiple of k0 (2) for QPSK Modulator
        % LCM(7, 2) = 14. So we floor to nearest multiple of 14.
        nCodedBitsNeeded = floor(nCodedBitsAvailable/14) * 14;
        ...
        % 2. Data Extraction
        % In AWGN, the channel H is effectively 1. No equalization needed.
...
%% Plot Results
figure;
semilogy(EbNo_dB, BER, 'g-o', 'LineWidth', 2, 'MarkerFaceColor', 'g');
grid on;
title('SISO AWGN (QPSK) with (7,4) Coding');
xlabel('Eb/N0 (dB)');
ylabel('Bit Error Rate (BER)');
legend('SISO AWGN QPSK');
ylim([1e-5 1]);
